...
vec3 water_fog(vec3 color, vec2 uv)
{
    ...
    float surface_d  = linearize_depth(gl_FragCoord.z);
...
void main()
{
    ...
    vec3 refract_dist = water_fog(texture(refraction_map, refract_uv).rgb, refract_uv);
    ...
    vec3 water  = mix(refract_dist, reflect_dist, fresnel);
    ...
    frag_albedo   = vec4(albedo, 0.5); //Water as pre-lit
