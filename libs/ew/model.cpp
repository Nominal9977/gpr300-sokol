...
namespace ew {
	ew::Mesh processAiMesh(aiMesh* aiMesh, bool instaced = false);

	Model::Model(const std::string& filePath, bool instaced)
	{
		...
		for (size_t i = 0; i < aiScene->mNumMeshes; i++)
		{
			...
			m_meshes.push_back(processAiMesh(aiMesh, instaced));
		}
	}
	...
	ew::Mesh processAiMesh(aiMesh* aiMesh, bool instaced) {
		...
		return ew::Mesh(meshData, instaced);
	}
