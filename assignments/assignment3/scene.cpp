...
Scene::Scene()
{
    ...
    toonShader = std::make_unique<ew::Shader>(
        "assets/shaders/default_shadow_instance.vs",
        "assets/shaders/worksesstion/toon_shadow.fs"
    );

    depth = std::make_unique<ew::Shader>(
    ...
    glBufferData(GL_ARRAY_BUFFER, 100*sizeof(glm::mat4), &InstanceModels[0], GL_STREAM_DRAW);
...
void  Scene::CacheInstanceData()
{
            ...
            for(auto x = -debug.width; x <= debug.width; x++)
            {
                ...
                {
                    auto postion = glm::vec3(x * debug.spacing, 0, y * debug.spacing);

                    auto matrix  = glm::translate(glm::mat4(1.0f), postion);

                    InstanceModels[i] = matrix;
                    i++;
                }
            }
}
...
void Scene::createDepthBuffer()
{
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, Shadow_fbo);
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT16, 800, 600, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_SHORT, nullptr);
...
void Scene::Render(void)
{
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);
    {
        ...
        //         for(auto y = -debug.width; y <= debug.width; y++)
        //         {
                    //toonShader->setMat4("model", InstanceModels[i]);
                    suzanne ->draw(100);
            //         i++;
            //     }
            // }
        ...
        // const auto plane_mat = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -2.0f, 0.0));
        // toonShader -> setMat4("model" , plane_mat);
        // plane.draw();
    }
